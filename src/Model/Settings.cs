    ...
    public class Settings
    {
        ...
        public static Settings Instance
        {
            get
            {
                if (_instance == null)
                {
                    var appData = Path.Combine(System.Environment.GetFolderPath(System.Environment.SpecialFolder.ApplicationData),
                                       "MongoSharp");
...
var ungrouped = (from x in collection
                   //where x.
                 select x).ToList();
...
Dump(grouped);"
    ...
    public class Preferences : IEditorPreferences
    {
        public Preferences()
        {
            EditorBackColor = System.Drawing.Color.White;
        ...
        {
            get { return EditorBackColor.ToArgb(); }
        ...
        public Preferences Clone()
        {
            return new Preferences
                {
                    EditorSyntaxLanguage = this.EditorSyntaxLanguage,
                    EditorBackColorAsArgb = this.EditorBackColorAsArgb,
                    EditorFontSize = this.EditorFontSize,
                    ResultGridFont = this.ResultGridFont == null ? null : this.ResultGridFont.Clone(),
                    OutputShowTimestamp = this.OutputShowTimestamp,
                    OutputTimestampFormat = this.OutputTimestampFormat,
                    OutputClearOnExecute = this.OutputClearOnExecute,
                    AllowAutoGeneratedModels = this.AllowAutoGeneratedModels,
                    ShowEditorLineNumbers = this.ShowEditorLineNumbers,
                    DefaultResultsView = this.DefaultResultsView
                };
        }
    }
