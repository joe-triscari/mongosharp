...
namespace MongoSharp
{
    public partial class EditorWindow : DockContent
    {
            ...
            scintillaLinqCode.CodePositionMapper = (text, pos) =>
                {
                    ...
                    if (collection != null && connection != null && database != null)
                    {
                        int injectedCodeStartPos = 0;

                        string code = new MongoCodeGenerator().GenerateMongoCode(database, collection, text,
                                                                                         mode, out injectedCodeStartPos);
        ...
        public Scintilla ScintillaEditor { get { return scintillaLinqCode; } }
        public bool CloseHandledByManager { get; set; 
        }
        public string SaveText
        {
            get { return IsNew ? TabText.Replace("*", "") : FileName; }
        }

        public string EditorText
        {
            get { return scintillaLinqCode.Text; }
                ...
                if (database != null)
                {
                    ...
                    if (collection != null)
                    {
                        ...
                        try
                        {
                            ...
                            {
                                toolStripComboBoxCollection.SelectedItem = null;
                                WindowManager.Instance.OutputWindow.AppendOutput(String.Format("Model for collection '{0}' has not been defined yet.", collection));
                            }
                            else
                            {
                        ...
                        catch (Exception ex)
                        {
                            ...
                            WindowManager.Instance.OutputWindow.Output = String.Format("{0}: {1}", "Error", ex.Message);
                        }
                        finally
                        {
        ...
        private void toolStripComboBoxDatabase_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                WindowManager.Instance.OutputWindow.Output = String.Format("{0}: {1}", "Connection Error", ex.Message);
            }
            finally
            {
                Cursor.Current = Cursors.Default;
                ...
                if (database != null)
                {
                    ...
                    if (collection != null)
                    {
                        try
                        {
                            ...
                            if (!collectionInfo.HasModel && !Settings.Instance.Preferences.AllowAutoGeneratedModels)
                                throw new Exception(String.Format("Model for collection '{0}' has not been defined yet.", collection));
        ...
        private void OnDropdownChanged()
        {
            ...
            toolStripButtonNavigateToColl.Enabled = connection != null && database != null && collection != null;
        }
                    ...
                    if (collection != null)
                    {
                        ...
                        {                            
                            ...
                            string executableQuery = mode == "Raw"
                                              ? rawQuery
                                              : new MongoCodeGenerator().GenerateMongoCode(database, collection, rawQuery, mode, out injectedCodeStartPos);
                            ...
                            _bgWorker.RunWorkerCompleted += (sender, e) =>
                                        {
                                            if (e.Error != null)
                                            {
                                                ...
                                                if (!err.Contains("Compile error"))
                                                    err = "Check your Model for correct datatypes:\r\n\r\n" + err;
        ...
        private void AddResultsTabPage(QueryResult queryResult)
        {
            ...
            var newTabPage = new TabPage(String.Format("Results - {0}", tabControlCollection.GetTabCountForText("Results") + 1));

            var queryResultControl = new UserControlQueryResults { Dock = DockStyle.Fill, OnCreateTableFromResults = (tbl) => AddTableResultsTabPage(tbl) };
        ...
        private void AddCodeViewTabPage(string code)
        {
            ...
            var page = new TabPage(String.Format("Code - {0}", tabControlCollection.GetTabCountForText("Code") + 1));
        ...
        {
            var page = new TabPage(String.Format("Table - {0}", tabControlCollection.GetTabCountForText("Table") + 1));
            var results = new UserControlCodeView { Dock = DockStyle.Fill };
            page.Controls.Add(results);
            tabControlCollection.TabPages.Add(page);
            page.Select();
